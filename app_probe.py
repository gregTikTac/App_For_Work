...
def list_OG_for_template(number_of_og):
    """Добавление  выбранных данных ОГ в список для рендеренга страницы"""
    counter = 0
    while list_SOG_and_OG_for_template != 1:

        try:
            number_of_og = int(number_of_og)
            if number_of_og == "" or number_of_og < 0:
                print('stop!')
                break
        ...
        try:
            if counter == number_of_og:
                list_SOG_and_OG_for_template.append(list_SOG_and_OG_for_template[counter])
        except ValueError:
            counter += 1
...
def choose_person_from_OG():
    list_OG_for_template(input('Введите номер ОГ: '))
    if len(list_SOG_and_OG_for_template) > 0:
        print(list_SOG_and_OG_for_template)
        print('+' + '---------------' * 10 + '+')
    else:
...
def choose_person_from_PAT():
    list_PAT_for_template(input('Введите номер в/с для ПАТ: '))
    if len(list_for_PAT_for_template) > 0:
        print(list_for_PAT_for_template)
        print('+' + '---------------' * 10 + '+')
    else:
        while len(list_for_PAT_for_template) < 7:
...
def choose_PM_from_OG():
    list_PM_for_template(input('Введите номер ПМ для ОГ: '))
...
def choose_AK_from_PAT():
    list_AK_for_template(input('Введите номер AK для ОГ: '))
